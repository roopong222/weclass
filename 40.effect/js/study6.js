$(document).ready(function(){
    ...
    $(document).mousemove(function(event){
        ...
        $(`.curTxt`).css(`top`,`${y-8}px`)
       
        // 보조커서가 나오는 소스
        $(`.cursorSub`).addClass(`on`)


        if($(`.cursorSubBox`).hasClass(`on`) == false){
            //마우스가 슬라이드 영역밖에 있다면
            ...
            $(`.cursorSub`).eq(1).css(`top`,`${y+35}px`)
            $(`.cursorSub`).eq(2).css(`top`,`${y+55}px`)
            $(`.cursorSub`).eq(3).css(`top`,`${y+75}px`)
            $(`.cursorSub`).eq(4).css(`top`,`${y+95}px`)
        }
